...
var menu_ALL = [
    ...
    ['menu_collapsedNowAnswer', '空白收起回答', '空白收起回答', true],
    ['menu_backToTop', '右键空白顶部', '右键空白回到顶部', true],
    ['menu_blockUsers', '屏蔽指定用户', '屏蔽指定用户',false],
    ['menu_customBlockUsers', '屏蔽用户列表', '自定义屏蔽用户', ['故事档案局', '盐选推荐', '盐选科普', '盐选成长计划', '知乎盐选会员', '知乎盐选创作者', '盐选心理', '盐选健康必修课', '盐选奇妙物语', '盐选生活馆', '盐选职场', '盐选文学甄选', '盐选作者小管家', '盐选博物馆', '盐选点金', '盐选测评室', '盐选科技前沿', '盐选会员精品']],
    ['menu_blockKeywords', '屏蔽指定短语', '屏蔽指定关键词',false],
    ['menu_customBlockKeywords', '屏蔽短语列表', '自定义屏蔽关键词', []],
    ['menu_blockType', '屏蔽指定类别', '勾选 = 屏蔽该类别的信息流', ''],
    ['menu_blockTypeVideo', '视频 [首页、搜索页、问题页]', '视频（首页、搜索页、问题页）', false],
    ...
    ['menu_blockYanXuan', '屏蔽盐选内容', '屏蔽盐选内容', true],
    ['menu_cleanSearch', '净化搜索热门', '净化搜索热门', false],
    ['menu_cleanTitles', '净化标题消息', '净化标题提醒', false],
...
function registerMenuCommand() {
    ...
    menu_ID[menu_ID.length] = GM_registerMenuCommand('💬 反馈 & 建议', function () {window.GM_openInTab('https://github.com/mika2048/ChromePlugins/', {active: true,insert: true,setParent: true});});
}
...
function menu_switch(menu_status, Name, Tips) {
    if (menu_status == 'true'){
        ...
        GM_notification({text: `已关闭 [${Tips}] 功能\n`, timeout: 1200, onclick: function(){location.reload();}});
    }else{
